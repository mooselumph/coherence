...
def create_layerwise_plan(params,rules,default_value=1):
...
def extract_masked_params(param, mask, flipped=False):
    if not flipped:
        if type(mask) == MaskFlag:
            if mask == MaskFlag.EXCLUDED:
                return []
            if mask == MaskFlag.INIT:
...
def global_threshold_prune(params,old_mask,fraction=0.1):
    ...
    def to_mask(w):
        return jnp.nonzero(jnp.abs(w) > threshold)

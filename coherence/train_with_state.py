...
def net_accuracy(net):  
    ...
    def accuracy(params: hk.Params, state: hk.State, batch: Batch) -> jnp.ndarray:
        predictions, _ = net.apply(params, state, batch["image"])
...
def update_params(opt,loss_fn):   
    ...
    def update(
      ...
      batch: Batch,
        ...
        grads, state = jax.grad(loss_fn,has_aux=True)(params, state, batch)
...
def do_training(
    ...
    for step in tqdm(range(epochs)):
        ...
        if aux_fn != None and step % aux_epoch == 0:
            aux_fn(params, state, batch)

        params, state, opt_state = update_fn(params, state, opt_state, batch)
...
def softmax_xent_loss(net):
    ...
    def loss(params, state, batch):
        logits, state = net.apply(params, state, batch["image"])
